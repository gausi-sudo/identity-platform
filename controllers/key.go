...
func (c *ApiController) GetKeys() {
	...
	if limit == "" || page == "" {
		keys, err := object.GetMaskedKeys(object.GetKeys(owner))
		if err != nil {
		...
		keys, err := object.GetMaskedKeys(object.GetPaginationKeys(owner, paginator.Offset(), limit, sortField, sortOrder))
		if err != nil {
			c.ResponseError(err.Error())
			return
		}
