...
class KeyListPage extends BaseListPage {
  renderTable(keys) {
    const columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            <Button type="primary" onClick={() => this.props.history.push(`/keys/${record.owner}/${record.name}`)}>
  ...
  fetch = (params = {}) => {
    ...
    KeyBackend.getKeys(Setting.getRequestOrganization(this.props.account), params.pagination.current, params.pagination.pageSize, sortField, sortOrder)
      .then((res) => {
        this.setState({
          loading: false,
        });
